<?php namespace OVillani\UserPermissions;

use Exception;
use Event;
use Backend;
use OVillani\UserPermissions\Models\Permission as PermissionModel;
...
class Plugin extends \System\Classes\PluginBase
{
    ...
    {
        return [
            'name' => 'ovillani.userpermissions::lang.plugin.name',
            'description' => 'ovillani.userpermissions::lang.plugin.description',
            'author' => 'JoakimBo',
    ...
    public function registerPermissions()
    {
        return [
            'ovillani.userpermissions.access_permissions' => [
                'tab'   => 'ovillani.userpermissions::lang.plugin.tab',
                'label' => 'ovillani.userpermissions::lang.plugin.access_permissions'
            ],
            'ovillani.userpermissions.access_user_permissions' => [
                'tab'   => 'ovillani.userpermissions::lang.plugin.tab',
                'label' => 'ovillani.userpermissions::lang.plugin.access_user_permissions'
            ],
            'ovillani.userpermissions.access_group_permissions' => [
                'tab'   => 'ovillani.userpermissions::lang.plugin.tab',
                'label' => 'ovillani.userpermissions::lang.plugin.access_group_permissions'
            ],
        ];
    }
    ...
    protected function extendWinterUserSideMenu()
    {
        Event::listen('backend.menu.extendItems', function($manager) {
            $manager->addSideMenuItems('Winter.User', 'user', [
                'permissions' => [
                    'label' => 'ovillani.userpermissions::lang.permissions.menu_label',
                    'icon' => 'icon-unlock',
                    'permissions' => ['ovillani.userpermissions.access_permissions'],
                    'url' => Backend::url('ovillani/userpermissions/permissions'),
                ]
            ]);
        });
    }
    ...
    protected function extendUserModel()
    {
        function getAllowedPermissions($model) {
            ...
            $groupPermissionsQuery = $model->user_permissions()->where('ovillani_userpermissions_user_permission.permission_state', 2)
            ->join('users_groups', 'ovillani_userpermissions_user_permission.user_id', '=', 'users_groups.user_id')
            ->join('ovillaniuserpermissions_group_permission', function ($join) {
                $join->on('users_groups.user_group_id', '=', 'ovillani_userpermissions_group_permission.group_id')
                ->on(
                    'ovillani_userpermissions_group_permission.permission_id',
                    '=',
                    'ovillani_userpermissions_user_permission.permission_id'
                )
                ->where('ovillani_userpermissions_group_permission.permission_state', '=', 1);
            })
            ->join('ovillani_userpermissions_permissions as permissions',
                'ovillani_userpermissions_group_permission.permission_id',
                '=',
                'permissions.id'
            )->select(
                'permissions.id',
                'permissions.code',
                'ovillani_userpermissions_user_permission.user_id',
                'ovillani_userpermissions_user_permission.permission_id',
                'ovillani_userpermissions_user_permission.permission_state',
                'ovillani_userpermissions_user_permission.created_at',
                'ovillani_userpermissions_user_permission.updated_at'
            );
        ...
        UserModel::extend(function($model)
        {
            $model->belongsToMany['user_permissions'] = [
                'OVillani\UserPermissions\Models\Permission',
                'table' => 'ovillani_userpermissions_user_permission',
                'key' => 'user_id',
            ...
            $model->addDynamicMethod(Config::get('ovillani.userpermissions::hasUserPermissionAlias', 'hasUserPermissionAlias'), function($permissionsInput, $match = 'all') use ($model) {
    ...
    protected function extendUserGroupModel()
    {
        UserGroupModel::extend(function($model) {
            $model->belongsToMany['user_permissions'] = ['OVillani\UserPermissions\Models\Permission',
                'table' => 'ovillani_userpermissions_group_permission',
                'key' => 'group_id',
                'otherKey' => 'permission_id',
                'timestamps' => true,
                'pivot' => ['permission_state'],
    ...
    protected function extendUserController()
    {
        Event::listen('backend.form.extendFields', function($widget) {
            ...
            if (BackendAuth::getUser()->hasAccess('ovillani.userpermissions.access_user_permissions')) {
                $widget->addTabFields([
                    'user_permissions' => [
                        'tab'   => 'ovillani.userpermissions::lang.permissions.menu_label',
    ...
    protected function extendUserGroupController()
    {
        Event::listen('backend.form.extendFields', function($widget) {
            ...
            // only add field if backend user has access
            if (BackendAuth::getUser()->hasAccess('ovillani.userpermissions.access_group_permissions')) {
                $widget->addTabFields([
                    'user_permissions' => [
                        'tab'   => 'ovillani.userpermissions::lang.permissions.menu_label',
                        'type'    => 'userpermissioneditor',
    ...
    {
        return [
            'OVillani\UserPermissions\FormWidgets\UserPermissionEditor' => 'userpermissioneditor',
        ];
    }
}
